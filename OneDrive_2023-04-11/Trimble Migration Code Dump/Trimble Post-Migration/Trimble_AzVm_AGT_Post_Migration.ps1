...
Function ProcessItemImpl($processor, $csvItem, $reportItem) {
 ...
 try {
    ...
    ############## Configuring Microsoft.Insights.VMDiagnosticsSettings ###################
         ...
         $processor.Logger.LogTrace("Going to Stop the VM"+" "+ $VirtualMachine.Name +" "+ "ResourceGroupName"+" "+$AzResourceGroupName)
...
############################## AzVm OS Disk Drive Expansion ###########################################
  ...
  try {
     ...
     $AzIPConfigsListOut = Invoke-AzVMRunCommand -ResourceGroupName $AzVMMachineName -VMName $VirtualMachine.Name -CommandId 'RunPowerShellScript' `
     ...
     $AzIPConfigsListOutFile | Out-File -FilePath "IPConfigOutput\$($AzVMMachineName)-DNS$((Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmss')).txt"

     $processor.Logger.LogTrace("IP Configuration Lists for DNS names resolved completed")
  }
  catch {
     $processor.Logger.LogTrace("Invoke-AzVMRunCommand: Check the IP Config List on $($AzVMMachineName) machine")
 }
   ####################################serveroutput###################################
   try {
        ...
        New-Item -Path 'IPConfigOutput' -ItemType Directory -Force
    $AzIPConfigsListOut = Invoke-AzVMRunCommand -ResourceGroupName $AzResourceGroupName -VMName $AzVMMachineName -CommandId 'RunPowerShellScript' `
        -ScriptString "net statistics workstation" -Verbose
    $processor.Logger.LogTrace($AzIPConfigsListOut.Value[0].Message)
    $AzIPConfigsListOutFile = $AzIPConfigsListOut.Value[0].Message
    $AzIPConfigsListOutFile | Out-File -FilePath "serveruptimeOutput\$($AzVMMachineName)-DNS$((Get-Date).ToUniversalTime().ToString(‘yyyyMMddTHHmmss’)).txt"

}
catch {
    $processor.Logger.LogTrace("Invoke-AzVMRunCommand: Check the serveruptime  on $($VirtualMachine.Name) machine")
