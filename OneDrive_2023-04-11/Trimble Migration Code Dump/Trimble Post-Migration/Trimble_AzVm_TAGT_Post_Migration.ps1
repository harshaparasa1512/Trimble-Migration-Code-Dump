...
Function ProcessItemImpl($processor, $csvItem, $reportItem) {
    ...
    try {
        ...
        $AzVmAGTPublicIPName = $csvItem.AZURE_TAGT_PUBLIC_IP_NAME.Trim()
        if ([string]::IsNullOrEmpty($AzVmAGTPublicIPName)) {
            $processor.Logger.LogError("AZURE_TAGT_PUBLIC_IP_NAME is not mentioned in the csv file")
            $reportItem.AdditionalInformation = "AZURE_TAGT_PUBLIC_IP_NAME is not mentioned in the csv file" 
            return
        }
        $AzVmAGTNSGName = $csvItem.AZURE_TAGT_NSG_NAME.Trim()
        if ([string]::IsNullOrEmpty($AzVmAGTNSGName)) {
            $processor.Logger.LogError("AZURE_TAGT_NSG_NAME is not mentioned in the csv file")
            $reportItem.AdditionalInformation = "AZURE_TAGT_NSG_NAME is not mentioned in the csv file" 
            return
        }
    ...
    # $AzVnetSubnet = Get-AzVirtualNetwork -ResourceGroupName $AzResourceGroupName -Name $AzVnetName | Get-AzVirtualNetworkSubnetConfig
    # Add-AzStorageAccountNetworkRule -ResourceGroupName $AzResourceGroupName -AccountName $AzStorageAccount.StorageAccountName -VirtualNetworkResourceId $AzVnetSubnet[0].Id
    ...
    ######################## Public IP Address & NSG Configuration in AGT server ###################################
    try {
        If ($AzVmAGTPublicIPName -eq '' -or $AzVmAGTPublicIPName -eq 'NA') {
            $processor.Logger.LogTrace("The Azure Virtual IP Address is not provided in input sheet")
        }
        elseif ($null -ne $AzVmAGTPublicIPName) {
            $AzVnetSubnet = Get-AzVirtualNetwork -ResourceGroupName $AzResourceGroupName -Name $AzVnetName | Get-AzVirtualNetworkSubnetConfig
            $AzVmNICPIP = Get-AzPublicIpAddress -Name $AzVmAGTPublicIPName -ResourceGroupName $AzResourceGroupName
            $AzVmNIC | Set-AzNetworkInterfaceIpConfig -PublicIpAddress $AzVmNICPIP.IpAddress -Subnet $AzVnetSubnet -Name $AzVmNIC.IpConfiguration.Name
            $AzVmNIC | Set-AzNetworkInterface

            $AzVmNSG = Get-AzNetworkSecurityGroup -ResourceGroupName $AzResourceGroupName -Name $AzVmAGTNSGName
            $AzVmNIC.NetworkSecurityGroup = $AzVmNSG
            Set-AzNetworkInterface -NetworkInterface $AzVmNIC
            $AzVmNIC | Set-AzNetworkInterface
        }
        else {
            $processor.Logger.LogTrace("The Azure Virtual IP Address is not provided in input sheet")
        ...
        $processor.Logger.LogTrace("The Azure Virtual IP Address is not valid in input sheet")
    }
