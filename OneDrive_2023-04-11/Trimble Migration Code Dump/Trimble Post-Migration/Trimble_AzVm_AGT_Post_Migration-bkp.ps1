...
Function ProcessItemImpl($processor, $csvItem, $reportItem) {
    ...
    try {
        ...
        $AzVmAGTPublicIPName = $csvItem.AZURE_AGT_PUBLIC_IP_NAME.Trim()
        if ([string]::IsNullOrEmpty($AzVmAGTPublicIPName)) {
            $processor.Logger.LogError("AZURE_AGT_PUBLIC_IP_NAME is not mentioned in the csv file")
            $reportItem.AdditionalInformation = "AZURE_AGT_PUBLIC_IP_NAME is not mentioned in the csv file" 
            return
        }
        $AzVmAGTNSGName = $csvItem.AZURE_AGT_NSG_NAME.Trim()
        if ([string]::IsNullOrEmpty($AzVmAGTNSGName)) {
            $processor.Logger.LogError("AZURE_AGT_NSG_NAME is not mentioned in the csv file")
            $reportItem.AdditionalInformation = "AZURE_AGT_NSG_NAME is not mentioned in the csv file" 
            return
        }
