class TrimbleAutomationLogger {
    ...
    TrimbleAutomationLogger([string] $CurrentCommandPath) {
        ...
        if (-not (Test-Path -LiteralPath $dirPathforLogs)) {            
            try {
                ...
                $fileName = [string]::Concat("Trimble_AzVm_Logs\", "log.", $this.ScriptName, ".", $this.FormatDate($this.StartDate), ".txt")
            }
            catch {                
...
Function New-TrimbleAutomation_LoggerInstance($CommandPath) {
    return [TrimbleAutomationLogger]::new($CommandPath)
}