class CsvProcessor {
    ...
    [string] GetCsvOutputPath($resolvedCsvPath) {
        ...
        $dirPathforLogs = [string]::Concat($dirPath, "\Trimble_AzVm_Logs")
        ...
        if (-not (Test-Path -LiteralPath $dirPathforLogs)) {            
            try {
                ...
                $CsvOutputFileName = [string]::Concat("Trimble_AzVm_Logs\", "output.", $scriptName, ".", $csvFileName, ".", $this.FormatDate($startDate), ".csv")
            }
            catch {                
    ...
    [void] ProcessFile($CsvFilePath) {
        ...
        foreach ($csvItem in $csvObj) {
            $reportItemInfo = [PSCustomObject]@{
                AZURE_SUBSCRIPTION_ID          = $($csvItem.AZURE_SUBSCRIPTION_ID)
                AZURE_PROJ_RESOURCE_GROUP_NAME = $($csvItem.AZURE_PROJ_RESOURCE_GROUP_NAME)
                VIRTUAL_MACHINE_NAME           = $($csvItem.SOURCE_MACHINE_NAME)
                EXCEPTION                      = $null
            }
            try {
                $this.EnsureSubscription($csvItem.AZURE_SUBSCRIPTION_ID)
